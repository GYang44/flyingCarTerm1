...
def create_grid(data, drone_altitude, safety_distance):
 ...
 for i in range(data.shape[0]):
     ...
     if ((drone_altitude <= alt + d_alt + safe_distance) and (drone_altitude >= alt - d_alt - safe_distance)):
         n = north_max - (north + d_north + safe_distance)
         if n < 0: n = int(0)
         else: n = int(np.floor(n))
         s = north_max - (north - d_north - safe_distance)
         if s > north_size: s = int(north_size)
         else: s = int(np.ceil(s))
